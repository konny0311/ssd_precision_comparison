    ...
    def parse_xml(self,
        ...
        for images_dir, image_set_filename, annotations_dir in zip(images_dirs, image_set_filenames, annotations_dirs):
            ...
            with open(image_set_filename) as f:
                ...
                # border_ids = int(len(image_ids) / 4)
                # image_ids = image_ids[:border_ids]
                # limitted_cnt = int(num_train/2)
                # if valid:
                #     random.shuffle(image_ids)
                #     image_ids = image_ids[:int(limitted_cnt / 2)]
                #     self.image_ids += image_ids
                # else:
                #     random.shuffle(image_ids)
                #     image_ids = image_ids[:limitted_cnt]
                self.image_ids += image_ids
            ...
            for image_id in it:
                ...
                # l:507のoforループ抜けたところで実行, クラス該当していたら追加する。
        ...
        print(list(class_cnts.keys()))
        print(list(class_cnts.values()))
        self.image_ids = tmp_image_ids
        self.dataset_size = len(self.filenames)
